  ...
  $sql="SELECT * FROM t_compte_cpt join t_profil_pfl using (cpt_pseudo) WHERE
    ...
    // La requête a echoué
      ...
      if($resultat->num_rows == 1) {
        ...
        $_SESSION['login']=$id;
        /* récupérer puis vérifier le rôle du profil dans la base MariaDB, puis affecter la valeur du rôle à $_SESSION['role']*/
        $compte=$resultat->fetch_assoc() ;
        $_SESSION['role']= $compte['pfl_role'] ;
        header("Location:admin_accueil.php");
      }else{
        $Error="pseudo/mot de passe incorrect(s) ou profil inconnu !";
        $Error1="<br /><a href=\"session.php\">Cliquez ici pour réafficher
        le formulaire</a>";
      }
    $mysqli->close();
  }
    ...
    <header id="header" class="hoc clear">
      ...
      <nav id="mainav" class="fl_right"> 
        ...
        <ul class="clear">
          ...
          <li><a class="drop" href="inscription.php"> Inscription </a> </li>
        </ul>
        <!-- ################################################################################################ -->
      </nav>
    </header>
